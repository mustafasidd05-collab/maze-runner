...
export default function GamePage() {
  ...
  const size  = useGameStore((s) => s.size  ?? 199);
  ...
  const handleGenerate = () => {
    ...
    const finalSize = size ?? 21;
    startGame(finalSeed, finalSize);
  };
    ...
    <div style={S.root}>
      ...
      {showHUD && (
        <div style={S.hudLayer}>
          ...
          <div style={S.hudStats}>
            ...
            <div style={S.glassCard}>
              <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                <span style={S.statLabel}>Seed</span>
                <input
                  value={seed ?? ""}
                  onChange={(e) => {
                    const val = Number(e.target.value);
                    setSeed(Number.isNaN(val) ? 0 : val);
                  }}
                  style={{
            ...
            {/* Moves + Timer performance metric */}
            <div style={S.glassCard}>
              <div style={{ display: "flex", gap: 12 }}>
                <div>
                  <div style={S.statLabel}>Moves</div>
                  <div style={S.statValue}>{moves}</div>
                </div>
                <div>
                  <div style={S.statLabel}>Time</div>
                  <div style={S.statValue}>{timer}</div>
                </div>
              </div>
            </div>
          </div>

          {/* Top-right: contextual game actions */}
          ...
          {/* Bottom-center: active contextual hints */}
